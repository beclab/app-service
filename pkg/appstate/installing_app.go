...
func (p *InstallingApp) IsOperating() bool {
...
func (p *InstallingApp) GetApp() *appsv1.Application {
	return p.app
}
...
func NewInstallingApp(ctx context.Context, client client.Client,
...
func (p *InstallingApp) Exec(ctx context.Context, c chan<- error) {
	...
	defer func() {
		...
		message := "app deployed and all container startup"
		if err != nil {
			state = appsv1.InstallFailed
			message = fmt.Sprintf("installing phase failed %v", err)
		}
		//FIXME: add opRecord if err
	...
	err = ops.Install()
	if err != nil {
		klog.Errorf("install app %s failed %v", p.manager.Spec.AppOwner, err)
		c <- err
		return
	}
	c <- nil
