...
func (h *Handler) suspend(req *restful.Request, resp *restful.Response) {
	...
	if !appstate.IsOperationAllowed(am.Status.State, v1alpha1.SuspendOp) {
		api.HandleBadRequest(resp, req, fmt.Errorf("%s operation is not allowed for %s state", v1alpha1.SuspendOp, am.Status.State))
		return
	}
