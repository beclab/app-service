// Code generated by client-gen. DO NOT EDIT.

package fake

import (
	"context"

	v1alpha1 "bytetrade.io/web3os/app-service/api/app.bytetrade.io/v1alpha1"
	v1 "k8s.io/apimachinery/pkg/apis/meta/v1"
	labels "k8s.io/apimachinery/pkg/labels"
	types "k8s.io/apimachinery/pkg/types"
	watch "k8s.io/apimachinery/pkg/watch"
	testing "k8s.io/client-go/testing"
)

// FakeAppImages implements AppImageInterface
type FakeAppImages struct {
	Fake *FakeAppV1alpha1
}

var appimagesResource = v1alpha1.SchemeGroupVersion.WithResource("appimages")

var appimagesKind = v1alpha1.SchemeGroupVersion.WithKind("AppImage")

// Get takes name of the appImage, and returns the corresponding appImage object, and an error if there is any.
func (c *FakeAppImages) Get(ctx context.Context, name string, options v1.GetOptions) (result *v1alpha1.AppImage, err error) {
	obj, err := c.Fake.
		Invokes(testing.NewRootGetAction(appimagesResource, name), &v1alpha1.AppImage{})
	if obj == nil {
		return nil, err
	}
	return obj.(*v1alpha1.AppImage), err
}

// List takes label and field selectors, and returns the list of AppImages that match those selectors.
func (c *FakeAppImages) List(ctx context.Context, opts v1.ListOptions) (result *v1alpha1.AppImageList, err error) {
	obj, err := c.Fake.
		Invokes(testing.NewRootListAction(appimagesResource, appimagesKind, opts), &v1alpha1.AppImageList{})
	if obj == nil {
		return nil, err
	}

	label, _, _ := testing.ExtractFromListOptions(opts)
	if label == nil {
		label = labels.Everything()
	}
	list := &v1alpha1.AppImageList{ListMeta: obj.(*v1alpha1.AppImageList).ListMeta}
	for _, item := range obj.(*v1alpha1.AppImageList).Items {
		if label.Matches(labels.Set(item.Labels)) {
			list.Items = append(list.Items, item)
		}
	}
	return list, err
}

// Watch returns a watch.Interface that watches the requested appImages.
func (c *FakeAppImages) Watch(ctx context.Context, opts v1.ListOptions) (watch.Interface, error) {
	return c.Fake.
		InvokesWatch(testing.NewRootWatchAction(appimagesResource, opts))
}

// Create takes the representation of a appImage and creates it.  Returns the server's representation of the appImage, and an error, if there is any.
func (c *FakeAppImages) Create(ctx context.Context, appImage *v1alpha1.AppImage, opts v1.CreateOptions) (result *v1alpha1.AppImage, err error) {
	obj, err := c.Fake.
		Invokes(testing.NewRootCreateAction(appimagesResource, appImage), &v1alpha1.AppImage{})
	if obj == nil {
		return nil, err
	}
	return obj.(*v1alpha1.AppImage), err
}

// Update takes the representation of a appImage and updates it. Returns the server's representation of the appImage, and an error, if there is any.
func (c *FakeAppImages) Update(ctx context.Context, appImage *v1alpha1.AppImage, opts v1.UpdateOptions) (result *v1alpha1.AppImage, err error) {
	obj, err := c.Fake.
		Invokes(testing.NewRootUpdateAction(appimagesResource, appImage), &v1alpha1.AppImage{})
	if obj == nil {
		return nil, err
	}
	return obj.(*v1alpha1.AppImage), err
}

// UpdateStatus was generated because the type contains a Status member.
// Add a +genclient:noStatus comment above the type to avoid generating UpdateStatus().
func (c *FakeAppImages) UpdateStatus(ctx context.Context, appImage *v1alpha1.AppImage, opts v1.UpdateOptions) (*v1alpha1.AppImage, error) {
	obj, err := c.Fake.
		Invokes(testing.NewRootUpdateSubresourceAction(appimagesResource, "status", appImage), &v1alpha1.AppImage{})
	if obj == nil {
		return nil, err
	}
	return obj.(*v1alpha1.AppImage), err
}

// Delete takes name of the appImage and deletes it. Returns an error if one occurs.
func (c *FakeAppImages) Delete(ctx context.Context, name string, opts v1.DeleteOptions) error {
	_, err := c.Fake.
		Invokes(testing.NewRootDeleteActionWithOptions(appimagesResource, name, opts), &v1alpha1.AppImage{})
	return err
}

// DeleteCollection deletes a collection of objects.
func (c *FakeAppImages) DeleteCollection(ctx context.Context, opts v1.DeleteOptions, listOpts v1.ListOptions) error {
	action := testing.NewRootDeleteCollectionAction(appimagesResource, listOpts)

	_, err := c.Fake.Invokes(action, &v1alpha1.AppImageList{})
	return err
}

// Patch applies the patch and returns the patched appImage.
func (c *FakeAppImages) Patch(ctx context.Context, name string, pt types.PatchType, data []byte, opts v1.PatchOptions, subresources ...string) (result *v1alpha1.AppImage, err error) {
	obj, err := c.Fake.
		Invokes(testing.NewRootPatchSubresourceAction(appimagesResource, name, pt, data, subresources...), &v1alpha1.AppImage{})
	if obj == nil {
		return nil, err
	}
	return obj.(*v1alpha1.AppImage), err
}
